...
public static class HermesUpdateJob {
    ...
    public static StartedProcess StartAssigned(string executable, string arguments, string workingDirectory) {
        ...
        try {
            ...
            if (!CreateProcess(executable, commandLine, IntPtr.Zero, IntPtr.Zero, true,
                    0x00000004 | 0x08000000, IntPtr.Zero, workingDirectory, ref si, out pi)) {
                int error = Marshal.GetLastWin32Error();
                throw new System.ComponentModel.Win32Exception(error, "CreateProcess failed (Win32 " + error + "): " + new System.ComponentModel.Win32Exception(error).Message);
            }
...
function Invoke-HermesStep([string]$Exe, [string[]]$HermesArgs, [string]$Tag) {
    ...
    try {
        ...
        $workingDirectory = if ($InstallRoot) { $InstallRoot } else { $PSScriptRoot }
...
if ($SelfTestPipeDrain) {
    ...
    $savedInstallRoot = $InstallRoot
    foreach ($invalid in @("cwd", "exe")) {
        $missing = Join-Path $TempDir "$stamp-missing"
        $launchExe = $powershell
        if ($invalid -eq "cwd") { $InstallRoot = $missing } else { $launchExe = $missing }
        $rejected = $false
        try { [void](Invoke-HermesStep $launchExe @("-NoProfile", "-Command", "exit 0") "invalid-launch") }
        catch {
            $rejected = $_.Exception.Message -match "CreateProcess failed \(Win32 [0-9]+\)"
            Write-Output $_.Exception.Message
        } finally { $InstallRoot = $savedInstallRoot }
        if (-not $rejected) { throw "Invalid $invalid launch did not fail immediately with Win32 diagnostic" }
    }
    Write-Output "LAUNCH-FAILURE SELF-TEST: PASS"
