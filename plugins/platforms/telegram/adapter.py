        ...
        try:
            ...
            for i, chunk in enumerate(chunks):
                outcome = await self._send_chunk_with_retries(
                    ...
                    reply_markup=fnos_failure_markup if i == len(chunks) - 1 else None)
    ...
    async def _handle_callback_query(self, update: "Update", context: "ContextTypes.DEFAULT_TYPE") -> None:
        ...
        if data.startswith("fnx:s:"):
            ...
            await self._reply_fnos_callback(
                query.message, cb["chat_id"], cb["chat_type"], cb["thread_id"], response_text
            )
            return
        ...
        if data.startswith("fnx:"):
            ...
            if action == "explain":
                try:
                    if query.message is None or cb["chat_id"] is None:
                        raise ValueError("cron failure callback has no source message")
                    clicked_at = datetime.now(timezone.utc)
                    report_date = getattr(query.message, "date", None)
