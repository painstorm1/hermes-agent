...
async def _send_telegram(token, chat_id, message, media_files=None, thread_id=None, disable_link_previews=False, force_document=False, metadata=None):
    ...
    try:
        ...
        fnos_markup = _fnos_cron_failure_markup(message, metadata)
        ...
        if _cap is not None and fnos_markup is None and utf16_len(formatted) <= _TELEGRAM_CAPTION_LIMIT:
        ...
        for index, chunk in enumerate(chunks):
            ...
            if fnos_markup is not None and index == len(chunks) - 1:
                chunk_kwargs["reply_markup"] = fnos_markup
