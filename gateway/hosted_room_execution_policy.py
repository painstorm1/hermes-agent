...
class RoomExecutionPolicy:
    ...
    def for_route(self, route: Mapping[str, Any] | None) -> "RoomExecutionPolicy":
        """Derive runtime-only authority; never widen a signed policy or route.

        Unlike a grant, a runtime restriction may deny even bot_room itself.
        The original signed mapping remains unchanged for grant verification.
        """
        toolsets = route.get("toolsets") if isinstance(route, Mapping) else None
        if not isinstance(toolsets, list):
            return self
        enabled = tuple(sorted(set(self.enabled_toolsets).intersection(toolsets)))
        unsigned = {k: v for k, v in self.as_mapping().items() if k != "policy_digest"}
        unsigned["enabled_toolsets"] = list(enabled)
        return replace(self, enabled_toolsets=enabled, policy_digest=_policy_digest(unsigned))
