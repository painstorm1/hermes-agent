...
def _final_response_from_result(result: dict, job_id: str, job_name: str, AIAgent) -> str:
    ...
    if isinstance(completion_reserve, dict) and result.get("completed") is not True:
        reason = completion_reserve.get("reason") or "incomplete"
        detail = result.get("error") or final_response_text
        raise RuntimeError(f"completion reserve unresolved ({reason})" + (f": {detail}" if detail else ""))
...
def run_job(
    ...
    try:
        ...
        agent._completion_reserve_turns = turns_for_cron_job(_cfg, job_id)
...
def _save_compose_deliver(
    ...
    with fence.side_effect_fence() as owns_output:
        ...
        d.saved_output_ref = Path(output_file).name
    if verbose:
    ...
    try:
        with fence.side_effect_fence() as owns_delivery:
            ...
            d.delivery_error = _deliver_result(
                ...
                output_ref=d.saved_output_ref,
                cron_process_failed=not d.success,
            )
...
def _deliver_crash_failure(
    job: dict, err_text: str, *, adapters, loop, output_ref=None,
    ...
    try:
        ...
        if (
            ...
            delivery_error, delivery_outcome = _deliver_crash_failure(
                ...
                output_ref=d.saved_output_ref if d is not None else None)
        try:
