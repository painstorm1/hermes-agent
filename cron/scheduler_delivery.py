...
def _live_route_metadata(t: _TargetDelivery) -> tuple[Optional[str], dict, dict]:
    ...
    route_metadata.update({
        **({"cron_output_ref": job["_cron_output_ref"]} if job.get("_cron_output_ref") else {}),
        **({"cron_process_failed": True} if job.get("_cron_process_failed") else {}),
    })
...
def _standalone_send(
    ...
    def _send():
        return _send_to_platform(
            ...
            media_files=media_files, metadata={
                "job_id": job["id"],
                **({"cron_output_ref": job["_cron_output_ref"]} if job.get("_cron_output_ref") else {}),
                **({"cron_process_failed": True} if job.get("_cron_process_failed") else {}),
            })
...
def _deliver_result(
    ...
    output_ref=None, cron_process_failed: bool = False,
) -> Optional[str]:
    ...
    job = dict(job)
    safe_output_ref = str(output_ref or job.get("_cron_output_ref") or "")
    job.pop("_cron_output_ref", None)
    if re.fullmatch(r"\d{4}-\d{2}-\d{2}_\d{2}-\d{2}-\d{2}\.md", safe_output_ref):
        job["_cron_output_ref"] = safe_output_ref
    if cron_process_failed:
        job["_cron_process_failed"] = True
