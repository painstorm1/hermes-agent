...
def _resolved_or_nothing(candidate: str, cwd: Optional[str], *, remote: bool = False) -> Iterator[Path | PurePosixPath]:
    """Yield *candidate* anchored on *cwd* (or the process cwd) when it is a real path."""
    if remote:
        if candidate.startswith("~") or "$" in candidate:
            raise ValueError("remote script path requires shell expansion")
        if candidate and "\x00" not in candidate:
            # Backend paths must never inherit the host drive, HOME or cwd.
            yield PurePosixPath(posixpath.join(cwd or "", candidate))
        return
...
def _references_at(segment: list[str], index: int, cwd: Optional[str], *, remote: bool = False) -> Iterator[Path | PurePosixPath]:
    ...
    if executable_name in {".", "source"}:
        if len(segment) > index + 1:
            yield from _resolved_or_nothing(segment[index + 1], cwd, remote=remote)
        return
    ...
    if executable_name in _SHELL_EXECUTABLES:
        ...
        if arg_index < len(arguments) and arguments[arg_index] not in _SHELL_COMMAND_FLAGS:
            yield from _resolved_or_nothing(arguments[arg_index], cwd, remote=remote)
        return
    ...
    if executable.strip("/") and ("/" in executable or executable.endswith((".sh", ".bash", ".zsh"))):
        yield from _resolved_or_nothing(executable, cwd, remote=remote)


def _iter_referenced_shell_scripts(command: str, *, cwd: Optional[str] = None, remote: bool = False) -> Iterator[Path | PurePosixPath]:
    ...
    for segment in _iter_command_segments(command):
        ...
        if index is None:
            continue
        yield from _references_at(segment, index, cwd, remote=remote)
        peeled = _peel_transparent_prefixes(segment, index)
        if peeled != index:
            yield from _references_at(segment, peeled, cwd, remote=remote)
...
def _sanitize_remote_script_text(
    ...
    """Apply the local-read contract to text from an untrusted ``read_remote_script`` callback:
    binary magic skips; oversized text fails closed before NUL stripping. Size compares re-encoded
    *bytes* (matching the ``head -c`` wire bound): a >1 MiB multibyte file truncated at the byte cap
    ...
    #76762, #77703.
    """
    if not text or _has_binary_magic(text[:_BINARY_SNIFF_BYTES].encode("utf-8", errors="replace")):
        return None, False
    byte_limit = _capped_read_limit(max_bytes)
...
def _contains_unsafe_gateway_action(
    ...
    for script_path in _iter_referenced_shell_scripts(command, cwd=cwd, remote=remote):
        # Do not touch a FileProvider path even to discover whether the file is hydrated.
        if not remote and _on_cloud_path(script_path):
            return True
        resolved = script_path if remote else _resolve_lenient(script_path)
        ...
        script_text, unsafe = (None, False) if remote else _read_referenced_script(script_path, max_bytes=budget.bytes_remaining)
        if unsafe:
            return True
        if script_text is None and read_remote_script is not None:
            # Read the authoritative backend, never a same-named host file.
            ...
            if remote_text is None:
                return True  # A referenced remote script could not be inspected.
            script_text, unsafe = _sanitize_remote_script_text(remote_text, max_bytes=budget.bytes_remaining)
            if unsafe:
                return True
        if not script_text:
            continue
        # A remote shell inherits its invocation cwd, not the script's directory.
        if recurse(script_text, cwd if remote else (_resolve_script_directory(str(resolved)) or cwd)):
            return True
    return False
...
def contains_gateway_lifecycle_command_or_referenced_script(
    ...
    try:
        ...
        if read_remote_script is not None:
            logger.warning("remote lifecycle script inspection failed closed", exc_info=True)
            return True
        logger.warning(
