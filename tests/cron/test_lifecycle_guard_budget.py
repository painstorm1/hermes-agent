...
def test_unique_path_budget_bounds_reads_and_fails_closed(monkeypatch, tmp_path):
    ...
    two = " && ".join(f"bash {shlex.quote((tmp_path / f's{i}.sh').as_posix())}" for i in range(2))
    three = " && ".join(f"bash {shlex.quote((tmp_path / f's{i}.sh').as_posix())}" for i in range(3))
...
def test_repeated_path_does_not_spend_unique_path_budget(monkeypatch, tmp_path):
    ...
    quoted = shlex.quote(script.as_posix())
    assert guard(f"bash {quoted} && bash {quoted} && sh {quoted}") is False
...
def test_line_budget_fails_closed_before_tokenizing_every_line(
    ...
    root = f"bash {shlex.quote(script.as_posix())}"
...
def test_default_budget_admits_a_wide_benign_wrapper_graph(tmp_path):
    ...
    hub = tmp_path / "hub.sh"
    hub.write_text("".join(f"bash {shlex.quote(c.as_posix())}\n" for c in children), encoding="utf-8")

    assert guard(f"bash {shlex.quote(hub.as_posix())}") is False
    ...
    hub.write_text(hub.read_text() + f"bash {shlex.quote(evil.as_posix())}\n", encoding="utf-8")
    assert guard(f"bash {shlex.quote(hub.as_posix())}") is True


@pytest.mark.parametrize("cwd, root", [("/remote", "bash a.sh"), (None, "bash /remote/a.sh")])
@pytest.mark.parametrize("leaf, blocked", [("echo ok", False), ("hermes gateway restart", True)])
def test_remote_nested_paths_never_use_host_filesystem(monkeypatch, cwd, root, leaf, blocked):
    child = "/remote/sub/b.sh" if cwd else "sub/b.sh"
    scripts = {"/remote/a.sh": "bash sub/b.sh", child: leaf}
    reads = []
    ...
    monkeypatch.setattr(lifecycle_guard, "_read_referenced_script", _explode)
    assert guard(root, cwd=cwd, read_remote_script=remote) is blocked
    assert reads == list(scripts)


@pytest.mark.parametrize("path", ["~/a.sh", "$HOME/a.sh", "/unreadable/a.sh"])
def test_uninspectable_remote_references_fail_closed(path):
    assert guard(f"bash {path}", read_remote_script=lambda _: None) is True


@pytest.mark.parametrize("case", ["dotdot", "nul", "cwd"])
def test_remote_reader_falsification_preserves_executed_script(case):
    dangerous = "hermes gateway restart\n"
    commands = {"dotdot": "bash /remote/link/../a.sh", "nul": "bash /scripts/a.sh", "cwd": "bash /scripts/a.sh"}
    scripts = {
        "dotdot": {"/remote/link/../a.sh": dangerous, "/remote/a.sh": "echo safe"},
        "nul": {"/scripts/a.sh": "#!/bin/bash\n# NUL: \x00\n" + dangerous},
        "cwd": {"/scripts/a.sh": "bash child.sh", "/scripts/child.sh": "echo safe", "/work/child.sh": dangerous},
    }[case]
    reads = []
    ...
    assert guard(commands[case], cwd="/work", read_remote_script=remote) is True
    expected = {"dotdot": ["/remote/link/../a.sh"], "nul": ["/scripts/a.sh"], "cwd": ["/scripts/a.sh", "/work/child.sh"]}
    assert reads == expected[case]


def test_remote_sanitizer_keeps_binary_skip_and_charges_nuls():
    sanitize = lifecycle_guard._sanitize_remote_script_text
    assert sanitize("MZ\x00" + "x" * 100, max_bytes=8) == (None, False)
    assert sanitize("\x7fELF\x00" + "x" * 100, max_bytes=8) == (None, False)
    assert sanitize("echo ok" + "\x00" * 20, max_bytes=8) == (None, True)
