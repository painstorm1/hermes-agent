...
@pytest.mark.asyncio
@pytest.mark.parametrize("field", ["hosted_room_dispatch", "_room_execution_policy"])
@pytest.mark.parametrize("value", [None, {}, {"enabled_toolsets": ["bot_room", "terminal"]}])
async def test_public_runs_rejects_ungranted_internal_room_fields(field, value):
    from gateway.platforms.base import PlatformConfig

    adapter = api_server.APIServerAdapter(PlatformConfig(enabled=True))
    request = MagicMock(headers={})
    request.json = AsyncMock(return_value={"input": "hello", field: value})
    adapter._create_agent = MagicMock(side_effect=AssertionError("must reject before launch"))
