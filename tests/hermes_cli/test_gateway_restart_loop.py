...
class TestTerminalToolGatewayLifecycleGuard:
    ...
    def test_blocks_lifecycle_command_hidden_in_referenced_script(
        ...
        result = json.loads(tt.terminal_tool(command=f"/bin/bash {shlex.quote(script.as_posix())}"))
    ...
    def test_blocks_launchctl_submit_inside_gateway(self, monkeypatch, tmp_path):
        ...
        result = json.loads(tt.terminal_tool(
            command=(
                ...
                f"/bin/bash {shlex.quote(script.as_posix())}"
            )
        ))
    ...
    def test_blocks_executable_shebang_script(self, monkeypatch, tmp_path):
        ...
        result = json.loads(tt.terminal_tool(command=shlex.quote(script.as_posix())))
    ...
    def test_shell_option_with_value_still_scans_script(self, monkeypatch, tmp_path):
        ...
        result = json.loads(tt.terminal_tool(
            command=f"/bin/bash -O extglob {shlex.quote(script.as_posix())}"
        ))
    ...
    def test_nested_wrapper_script_is_scanned(self, monkeypatch, tmp_path):
        ...
        result = json.loads(tt.terminal_tool(command=f"/bin/bash {shlex.quote(outer.as_posix())}"))

        assert result["exit_code"] == 1

    @pytest.mark.skipif(not hasattr(os, "mkfifo"), reason="FIFO API is unavailable on this platform")
    def test_non_regular_referenced_script_fails_closed(self, monkeypatch, tmp_path):
        ...
        result = json.loads(tt.terminal_tool(command=f"/bin/bash {shlex.quote(fifo.as_posix())}"))
    ...
    def test_safe_referenced_script_passes_through(self, monkeypatch, tmp_path):
        ...
        command = f"/bin/bash {shlex.quote(script.as_posix())}"
...
class TestLifecycleGuardModule:
    ...
    def test_dot_operator_sourced_script_is_scanned(self, tmp_path):
        ...
        script.write_text("#!/bin/bash\nhermes gateway restart\n")
        assert (
            contains_gateway_lifecycle_command_or_referenced_script(f". {shlex.quote(script.as_posix())}")
            is True
        )
    ...
    def test_nul_padded_script_is_still_scanned(self, tmp_path):
        ...
        assert (
            contains_gateway_lifecycle_command_or_referenced_script(f"bash {shlex.quote(script.as_posix())}")
            is True
        )
    ...
    def test_source_builtin_sourced_script_is_scanned(self, tmp_path):
        ...
        script.write_text("#!/bin/bash\nhermes gateway restart\n")
        assert (
            contains_gateway_lifecycle_command_or_referenced_script(f"source {shlex.quote(script.as_posix())}")
            is True
        )
    ...
    def test_elf_binary_is_not_scanned_as_script(self, tmp_path):
        ...
        assert (
            contains_gateway_lifecycle_command_or_referenced_script(f"{shlex.quote(binary.as_posix())} --version")
            is False
        )
    ...
    def test_macho_binary_is_not_scanned_as_script(self, tmp_path):
        ...
        for name, magic in (
            ...
            assert (
                contains_gateway_lifecycle_command_or_referenced_script(
                    f"{shlex.quote(binary.as_posix())} --version"
                )
                is False
            )
    ...
    @pytest.mark.skipif(os.name == "nt", reason="Windows symlinks require elevated privileges; exercised on POSIX")
...
class TestDotSourceIsScannedLikeSource:
    ...
    def test_both_spellings_block_a_referenced_script(self, tmp_path, helper, form):
        assert self._scan(form.format(path=shlex.quote(helper.as_posix())), cwd=str(tmp_path)) is True
    ...
    def test_env_assignment_prefix_does_not_hide_dot_source(self, tmp_path, helper):
        assert self._scan(f"FOO=1 . {shlex.quote(helper.as_posix())}", cwd=str(tmp_path)) is True

    def test_dot_source_nested_in_shell_c_is_blocked(self, tmp_path, helper):
        assert self._scan("sh -c " + shlex.quote(". " + shlex.quote(helper.as_posix())), cwd=str(tmp_path)) is True
    ...
    def test_sourcing_a_clean_script_is_allowed(self, tmp_path):
        ...
        assert self._scan(f". {shlex.quote(clean.as_posix())}", cwd=str(tmp_path)) is False
...
class TestTransparentWrapperPrefixes:
    ...
    def test_wrapped_script_reference_is_scanned(self, tmp_path, helper, prefix):
        assert self._scan(f"{prefix} bash {shlex.quote(helper.as_posix())}", cwd=str(tmp_path)) is True
    ...
    def test_wrapped_dot_source_is_scanned(self, tmp_path, helper, prefix):
        assert self._scan(f"{prefix} . {shlex.quote(helper.as_posix())}", cwd=str(tmp_path)) is True

    @pytest.mark.parametrize("prefix", ["sudo", "env", "nohup"])
    def test_wrapped_shell_c_payload_is_scanned(self, tmp_path, helper, prefix):
        assert self._scan(
            f"{prefix} sh -c " + shlex.quote("bash " + shlex.quote(helper.as_posix())), cwd=str(tmp_path)
        ) is True
    ...
    @pytest.mark.parametrize("command", [
        ...
        "su -c {payload}",
        "su root -c {payload}",
        "runuser -u root -c {payload}",
    ])
    def test_command_string_options_are_rescanned(self, tmp_path, helper, command):
        assert self._scan(command.format(payload=shlex.quote("bash " + shlex.quote(helper.as_posix()))), cwd=str(tmp_path)) is True
    ...
    def test_local_script_named_like_a_wrapper_is_still_scanned(
        ...
        assert self._scan(shlex.quote(script.as_posix()), cwd=str(tmp_path)) is True

    def test_wrapped_clean_script_is_allowed(self, tmp_path):
        clean = tmp_path / "ok.sh"
        clean.write_text("#!/bin/sh\necho hi\n", encoding="utf-8")
        assert self._scan(f"sudo bash {shlex.quote(clean.as_posix())}", cwd=str(tmp_path)) is False
...
class TestLifecycleGuardNeverRaises:
    ...
    def test_command_referencing_elf_binary_returns_false(self, tmp_path):
        ...
        assert self._scan(f"{shlex.quote(binary.as_posix())} -m json.tool /tmp/x.json") is False
    ...
    def test_non_utf8_referenced_file_never_raises(self, tmp_path):
        ...
        assert self._scan(f"bash {shlex.quote(weird.as_posix())}") is False
    ...
    def test_sourced_zshrc_docker_completions_dir_is_not_blocked(self, tmp_path):
        ...
        zshrc.write_text(
            f"fpath=({shlex.quote(completions.as_posix())} /usr/local/share/zsh/site-functions $fpath)\n",
            encoding="utf-8",
        )
        assert self._scan(f"source {shlex.quote(zshrc.as_posix())}") is False
    ...
    def test_directory_and_dev_null_fail_closed_not_crash(self, tmp_path):
        ...
        assert self._scan(f"bash {shlex.quote(tmp_path.as_posix())}") is False
        if os.name != "nt":
