...
class TestNodeRuntimeNpmResolution:
    ...
    def test_update_rebuilds_desktop_that_disappears_mid_update(self):
        ...
        from tests.hermes_cli.test_update_desktop_rebuild import _write_complete_package, _valid_pe

        def build_complete(*_args, **_kwargs):
            _write_complete_package(packaged_exe.parent, _valid_pe())
        ...
        with (
            ...
            patch.object(hm, "_run_logged_subprocess", side_effect=build_complete) as desktop_build,
        ):
    ...
    def test_git_failure_zip_fallback_rebuilds_missing_desktop(self, tmp_path, monkeypatch):
        ...
        assert packaged_exe.read_bytes() == bytes(executable)
