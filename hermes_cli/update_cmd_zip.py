...
def _stage_replacement(src: str, dst: str) -> str:
    from hermes_cli.update_cmd import _stage_replacement as preserve_and_stage
    return preserve_and_stage(src, dst)
...
def _require_staging_space(extracted: str, entries: list[str], project_root: str, *, preservation_plan=None) -> None:
    from hermes_cli.update_cmd import _zip_staging_size
    need = _zip_staging_size(Path(extracted), entries, Path(project_root), preservation_plan=preservation_plan)
...
def _stage_entries(extracted: str, entries: list[str], project_root: str, *, preservation_plan=None) -> list[tuple[str, str]]:
    from hermes_cli.update_cmd import _stage_zip_entries
    return _stage_zip_entries(Path(extracted), Path(project_root), entries, preservation_plan=preservation_plan)
...
def _download_and_swap_zip(branch: str, zip_url: str) -> None:
    ...
    try:
        ...
        from hermes_cli.update_cmd import _build_zip_preservation_plan
        preservation_plan = _build_zip_preservation_plan(Path(project_root), entries)
        _require_staging_space(extracted, entries, project_root, preservation_plan=preservation_plan)
        staged = _stage_entries(extracted, entries, project_root, preservation_plan=preservation_plan)
        try:
